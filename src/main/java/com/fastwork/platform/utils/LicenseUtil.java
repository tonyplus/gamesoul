...
public class LicenseUtil {
	...
	*/
	...
	public static boolean validateLicense(String license) throws Exception {
		...
		if (arr.length == 2) {
			if (!"999".equals(arr[1])) {
				Calendar registCalendar = Calendar.getInstance();
				registCalendar.setTimeInMillis(Long.parseLong(arr[1]));
				Calendar currCalendar = Calendar.getInstance();
				int i = registCalendar.compareTo(currCalendar);
				if (i < 0) {
					return false;
				}
			}

			String encrypt = createLicense(Encrypter
					.encrypt(mac, Encrypter.MD5));
			if (encrypt.equals(arr[0])) {
				return true;
			} else {
				return false;
			}
