...
(function() {
	...
	A = J();
	if ($.$wdate)
		K(BASE_URL+ "/css/lib/datepicker/WdatePicker.css");
	U = X;
	if ($.$crossFrame) {
		try {
	...
	function J() {
		...
		for ( var B = 0; B < $.length; B++) {
			...
			_ = _.substr(0, _.toLowerCase().indexOf(BASE_URL+"/js/lib/wdatepicker.js"));
			A = _.lastIndexOf("/");
			if (A > 0)
				_ = _.substring(0, A + 1);
			if (_)
	...
	function I(G, _) {
		...
		function E(F, E) {
			...
			var G = [
					...
					"</script><script src=", BASE_URL, "/js/lib/datepicker/lang/", H.name,
					".js charset=", H.charset, "></script>" ];
			for ( var I = 0; I < C.length; I++)
			//	if (C[I].name == E.skin)
					G.push("<link rel=\"stylesheet\" type=\"text/css\" href=\""
							+ BASE_URL + "/css/lib/datepicker/datepicker.css\" charset=\"" + C[I].charset
							+ "\"/>");
			G
					.push("<script type=\"text/javascript\" src=\""
						
							+ BASE_URL+"/js/lib/datepicker/calendar.js?\"+Math.random()+\" charset=\"gb2312\"></script>");
			G
